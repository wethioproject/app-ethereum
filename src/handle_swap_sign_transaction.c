...
void handle_swap_sign_transaction(create_transaction_parameters_t* sign_transaction_params, chain_config_t *config, txContext_t* txContext) {
    //copy_transaction_parameters(sign_transaction_params);
    chainConfig = config;
    btchip_context_init();
    txContext->called_from_swap = 1;
    io_seproxyhal_init();
    UX_INIT();
    USB_power(0);
    USB_power(1);
